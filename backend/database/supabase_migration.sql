...
-- ============================================
-- ENABLE ROW LEVEL SECURITY (optional, permissive for now)
-- ============================================

-- For now, allow full access via anon key (no auth yet)
-- You can add RLS policies later when you add authentication
...
-- Also for tables that may exist
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'maintenance_log') THEN
        ALTER TABLE maintenance_log ENABLE ROW LEVEL SECURITY;
        EXECUTE 'CREATE POLICY "Allow all" ON maintenance_log FOR ALL USING (true) WITH CHECK (true)';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'maintenance_types') THEN
        ALTER TABLE maintenance_types ENABLE ROW LEVEL SECURITY;
        EXECUTE 'CREATE POLICY "Allow all" ON maintenance_types FOR ALL USING (true) WITH CHECK (true)';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'roles') THEN
        ALTER TABLE roles ENABLE ROW LEVEL SECURITY;
        EXECUTE 'CREATE POLICY "Allow all" ON roles FOR ALL USING (true) WITH CHECK (true)';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        ALTER TABLE users ENABLE ROW LEVEL SECURITY;
        EXECUTE 'CREATE POLICY "Allow all" ON users FOR ALL USING (true) WITH CHECK (true)';
    END IF;
END $$;
